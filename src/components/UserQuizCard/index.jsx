  ...
  return (
    ...
    >
      ...
      >
        ...
        {subHeading && (
          <Typography sx={{ ...Style.subHeading }}>{subHeading}</Typography>
        )}
        {isUser ? (
          <ArrowForwardIcon sx={Style.nextIcon} />
        ) : (
          isAdmin &&
          !item?.upload_status && (
            <CustomButton
              ...
              sx={{ ...Style.nextIcon, ...Style.uploadButton }}
              typSx={{
        ...
        {score === 0 && typeof score === "number" && (
          <Typography
            sx={{ ...Style.headings, ...headingStyle, fontSize: "18px" }}
          >
            Retry
          </Typography>
        )}
      </Box>
      {(!isUser || item?.result_status) && (
        <Box className="hover-buttons" sx={Style.btnConatiner}>
          ...
          {(item?.upload_status || isUser) && (
            <CustomButton
              ...
              buttonText={
                isAdmin ? "Results" : isUser ? "View Results" : `SHOW RESULTS`
              }
            />
          )}
        </Box>
