...
const UploadDocumentContent = ({
  ...
  useEffect(() => {
    if (!!file) {
      if (isNew) setTempFileName(file?.name?.split(".pdf")[0]);
    ...
    >
      <Box sx={Style.photoContainer}>
        <Box
          component={"img"}
          src={
            !!Object.values(editItem)?.length && !photo
              ? IMAGES.document
              : photo || IMAGES.image
          }
          sx={Style.documentImg}
        />
        <Button
          sx={Style.uploadFile}
          component="label"
          ...
          tabIndex={-1}
          startIcon={
            <Box component="img" src={IMAGES.thumbnail} sx={Style.thumbnail} />
          }
        >
      ...
      <Box component={"form"} sx={Style.form(isNew)} onSubmit={handleSubmit}>
        <Box sx={Style.formControlContainer}>
          ...
          <OutlinedInput
            ...
            onChange={(e) => {
              if (isNew) setTempFileName(e?.target?.value?.split(".pdf")[0]);
        ...
        <Box sx={Style.formControlContainer}>
          ...
          <textarea
            ...
            value={overview} // Bind state to the textarea
            onChange={handleChange} // Set the handler
          />
        </Box>
        <Box
          sx={{ ...Style.flexContainer, height: { xs: "178px", md: "82px" } }}
        >
          <Box sx={{ ...Style.formControlContainer, height: "80px" }}>
            <Typography sx={Style.dueDatePara}>
              ...
              <Typography
                sx={{ ...Style.heading, ...Style.optional }}
                component={"span"}
              >
                {`(Optional)`}
              </Typography>
          ...
          <Box sx={Style.formControlContainer}>
            ...
            <FormControl
              sx={{ width: { xs: "100%", md: "100%" }, margin: 0, mt: 1 }}
              variant="outlined"
            >
              <OutlinedInput
                sx={{
        ...
        <Box>
          ...
          <Typography component={"ul"} sx={Style.listMain}>
            <Typography component={"li"} sx={Style.subHeading}>
              Quiz
            </Typography>
            <Typography component={"li"} sx={Style.subHeading}>
              Summary
            </Typography>
            <Typography component={"li"} sx={{ ...Style.subHeading, mr: 0 }}>
              Key Points
            </Typography>
          </Typography>
        </Box>
